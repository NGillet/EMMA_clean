...
#endif
#ifdef S_X
        E0overI[igrp] = param->atomic.hnu[igrp] / hnu0;
#endif
    }
        ...
        for(icell = 0; icell < 8; icell++) { // we scan the cells
            ...
            */
            ...
            while(currentcool_t < dt)
            {
                ...
                if(fudgecool < 1e-6) {
                    printf("ERROR : chemrad can't converge (fudgecool<1e-6)\n");
...
//	    fzt[igrp]=floc[2+idloc3+igrp*BLOCKCOOL*3]*EE;
                ...
                for(igrp = 0; igrp < NGRP; igrp++) {
                    ai_tmp1 += alphai[igrp] * et[igrp] * (!chemonly);
                }

#else

                for(igrp = 0; igrp < NGRP; igrp++) {
                    ai_tmp1 += alphai[igrp] * egyloc[idloc + igrp * BLOCKCOOL] * (!chemonly);
                }
                ...
                for(igrp = 0; igrp < NGRP; igrp++) {
                    ai_tmp1 += alphai[igrp] * et[igrp] * N2[igrp] * (!chemonly);
                }

#else

                for(igrp = 0; igrp < NGRP; igrp++) {
                    ai_tmp1 += alphai[igrp] * egyloc[idloc + igrp * BLOCKCOOL] * N2[igrp] * (!chemonly);
                }
                ...
                if(compcool) {
                    ...
                    for(igrp = 0; igrp < NGRP; igrp++) {
                        ai_tmp1 += egyloc[idloc + igrp * BLOCKCOOL] * (alphae[igrp] * hnu[igrp] - (alphai[igrp] * hnu0)) * (!chemonly);
                    }
                    ...
                    for(igrp = 0; igrp < NGRP; igrp++) {
                        ai_tmp1 += et[igrp] * (alphae[igrp] * hnu[igrp] - (alphai[igrp] * hnu0)) * F2[igrp] * (!chemonly);
                    }
                    ...
                    for(igrp = 0; igrp < NGRP; igrp++) {
                        ai_tmp1 += egyloc[idloc + igrp * BLOCKCOOL] * (alphae[igrp] * hnu[igrp] - (alphai[igrp] * hnu0)) * F2[igrp] * (!chemonly);
                    }
