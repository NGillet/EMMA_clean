...
void readAtomic(struct RUNPARAMS* param) {
    ...
    for(i = 0; i < param->atomic.n; i++) {
        rstat = fscanf(buf, "%lf", &dummy);
        param->atomic.hnu[i] = dummy * ELECTRONVOLT;
        rstat = fscanf(buf, "%lf", &dummy);
        param->atomic.alphae[i] = param->clightorg * (dummy * LIGHT_SPEED_IN_M_PER_S);
        rstat = fscanf(buf, "%lf", &dummy);
        param->atomic.alphai[i] = param->clightorg * (dummy * LIGHT_SPEED_IN_M_PER_S);
#ifdef HESIMPLE
        rstat = fscanf(buf, "%lf", &dummy);
        param->atomic.alphaeHE[i] = param->clightorg * (dummy * LIGHT_SPEED_IN_M_PER_S);
        rstat = fscanf(buf, "%lf", &dummy);
        param->atomic.alphaiHE[i] = param->clightorg * (dummy * LIGHT_SPEED_IN_M_PER_S);
        rstat = fscanf(buf, "%lf", &dummy);
        param->atomic.alphaeHE2[i] = param->clightorg * (dummy * LIGHT_SPEED_IN_M_PER_S);
        rstat = fscanf(buf, "%lf", &dummy);
        param->atomic.alphaiHE2[i] = param->clightorg * (dummy * LIGHT_SPEED_IN_M_PER_S);
#endif
        rstat = fscanf(buf, "%lf", &dummy);
        param->atomic.factgrp[i] = dummy;
...
void ReadParameters(char* fparam, struct RUNPARAMS* param) {
    ...
    else
    {
        ...
        if (debug) printf("param->ngridmax=%d\n", param->ngridmax);

        rstat = fscanf(buf, "%s %d", stream, &param->npartmax);

        if (debug) printf("param->npartmax=%d\n", param->npartmax);

        rstat = fscanf(buf, "%s %d", stream, &param->nbuff);

        if (debug) printf("param->nbuff=%d\n", param->nbuff);

        rstat = fscanf(buf, "%s", stream);
        rstat = fscanf(buf, "%s %d", stream, &param->ndumps);

        if (debug) printf("param->ndumps=%d\n", param->ndumps);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->dt_dump = (REAL)dummyf;

        if (debug) printf("param->dt_dump=%e\n", param->dt_dump);

        rstat = fscanf(buf, "%s %d", stream, &param->nsteps);

        if (debug) printf("param->nsteps=%d\n", param->nsteps);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->dt = (REAL)dummyf;

        if (debug) printf("param->dt=%e\n", param->dt);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->tmax = (REAL)dummyf;

        if (debug) printf("param->tmax=%e\n", param->tmax);

        rstat = fscanf(buf, "%s", stream);
        rstat = fscanf(buf, "%s %d", stream, &param->lcoarse);

        if (debug) printf("param->lcoarse=%d\n", param->lcoarse);

        rstat = fscanf(buf, "%s %d", stream, &param->lmax);

        if (debug) printf("param->lmax=%d\n", param->lmax);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->amrthresh0 = (REAL)dummyf;

        if (debug) printf("param->amrthresh=%e\n", param->amrthresh0);

        rstat = fscanf(buf, "%s %d", stream, &param->nsmooth);

        if (debug) printf("param->nsmooth=%d\n", param->nsmooth);

        rstat = fscanf(buf, "%s", stream);
        rstat = fscanf(buf, "%s %d", stream, &param->DM_res);

        if (debug) printf("param->DM_res=%d\n", param->DM_res);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->dx_res = (REAL)dummyf;

        if (debug) printf("param->dx_res=%e\n", param->dx_res);

        rstat = fscanf(buf, "%s", stream);
        rstat = fscanf(buf, "%s %d", stream, &param->niter);

        if (debug) printf("param->niter=%d\n", param->niter);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->poissonacc = (REAL)dummyf;

        if (debug) printf("param->poissonacc=%e\n", param->poissonacc);

        rstat = fscanf(buf, "%s %d", stream, &param->mgridlmin);

        if (debug) printf("param->mgridlmin=%d\n", param->mgridlmin);
        ...
        if (debug) printf("param->nvcycles=%d\n", param->nvcycles);

        rstat = fscanf(buf, "%s %d", stream, &param->nrelax);

        if (debug) printf("param->nrelax=%d\n", param->nrelax);

        rstat = fscanf(buf, "%s", stream);
        rstat = fscanf(buf, "%s %d", stream, &param->nrestart);

        if (debug) printf("param->nrestart=%d\n", param->nrestart);

        rstat = fscanf(buf, "%s", stream);
        rstat = fscanf(buf, "%s %d", stream, &param->gstride);

        if (debug) printf("param->gstride=%d\n", param->gstride);

        rstat = fscanf(buf, "%s %d", stream, &param->hstride);

        if (debug) printf("param->hstride=%d\n", param->hstride);

        rstat = fscanf(buf, "%s %d", stream, &param->nsubcycles);

        if (debug) printf("param->nsubcycles=%d\n", param->nsubcycles);

        rstat = fscanf(buf, "%s %d", stream, &param->nthread);

        if (debug) printf("param->nthread=%d\n", param->nthread);

        rstat = fscanf(buf, "%s %d", stream, &param->nstream);

        if (debug) printf("param->nstream=%d\n", param->nstream);

        rstat = fscanf(buf, "%s %d", stream, &param->ompthread);

        if (debug) printf("param->ompthread=%d\n", param->ompthread);

        rstat = fscanf(buf, "%s", stream);
#ifdef WRAD
        rstat = fscanf(buf, RF, stream, &dummyf);
        param->clight = (REAL)dummyf;
        param->clightorg = (REAL)dummyf;

        if (debug) printf("param->clight=%e\n", param->clight);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->denthresh = (REAL)dummyf;

        if (debug) printf("param->denthresh=%e\n", param->denthresh);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->tmpthresh = (REAL)dummyf;

        if (debug) printf("param->tmpthresh=%e\n", param->tmpthresh);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->srcint = (REAL)dummyf;

        if (debug) printf("param->srcint=%e\n", param->srcint);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->fesc = (REAL)dummyf;

        if (debug) printf("param->fesc=%e\n", param->fesc);

        param->srcint *= param->fesc;
        char filename[256];
        rstat = fscanf(buf, "%s %s", stream, filename);
        sprintf(param->atomic.path, "./SRC/src/atomic_data/%s", filename);

        if (debug) printf("param->atomic.path=%s\n", param->atomic.path);
...
#ifdef STARS
        rstat = fscanf(buf, RF, stream, &dummyf);
        param->stars->overdensity_cond = (REAL)dummyf;

        if (debug) printf("param->stars->overdensity_cond=%e\n", param->stars->overdensity_cond);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->stars->density_cond = (REAL)dummyf;

        if (debug) printf("param->stars->density_cond=%e\n", param->stars->density_cond);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->stars->efficiency = (REAL)dummyf;

        if (debug) printf("param->stars->efficiency=%e\n", param->stars->efficiency);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->stars->tlife = (REAL)dummyf;

        if (debug) printf("param->stars->tlife=%e\n", param->stars->tlife);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->stars->mass_res = (REAL)dummyf;

        if (debug) printf("param->stars->mass_res=%e\n", param->stars->mass_res);
...
#ifdef SUPERNOVAE
        rstat = fscanf(buf, RF, stream, &dummyf);
        param->sn->feedback_eff	= (REAL)dummyf;

        if (debug) printf("param->sn->feedback_eff=%e\n", param->sn->feedback_eff);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->sn->feedback_frac	= (REAL)dummyf;

        if (debug) printf("param->sn->feedback_frac=%e\n", param->sn->feedback_frac);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->sn->ejecta_proportion	= (REAL)dummyf;

        if (debug) printf("param->sn->ejecta_proportion=%e\n", param->sn->ejecta_proportion);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->sn->sn_egy	= (REAL)dummyf;

        if (debug) printf("param->sn->sn_egy=%e\n", param->sn->sn_egy);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->sn->tlife	= (REAL)dummyf;

        if (debug) printf("param->sn->tlife=%e\n", param->sn->tlife);

        rstat = fscanf(buf, RF, stream, &dummyf);
        param->sn->load_factor = (REAL)dummyf;

        if (debug) printf("param->sn->load_factor=%e\n", param->sn->load_factor);
