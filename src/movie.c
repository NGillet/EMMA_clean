...
//------------------------------
    ...
    if(strcmp(param->movie->mode_str, "max") == 0) {
        mode = 0;
    }

    if(strcmp(param->movie->mode_str, "avg") == 0) {
        mode = 1;
    }
        ...
        for(iOct = 0; iOct < cpu->locNoct[ilev - 1]; iOct++) {
            ...
            for(icell = 0; icell < 8; icell++) {
                ...
                if( cell->child == NULL || ilev == lmap ) {
                    ...
                    for(kk = 0; kk < locN; kk++) {
                        if((kmap + kk) >= (nmapz + k0)) continue;

                        for(jj = 0; jj < locN; jj++) {
                            if((jmap + jj) >= (nmapy + j0))	continue;

                            for(ii = 0; ii < locN; ii++) {
                                if((imap + ii) >= (nmapx + i0)) continue;
